...
try {
    $stmt = $conn->prepare("SELECT testimonial_id, user_id FROM testimonials WHERE testimonial_id = :id AND status = 'Pending'");
    $stmt->bindParam(":id", $testimonialID);
    $stmt->execute();
    $testimonial = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$testimonial) {
        echo json_encode(["success" => false, "message" => "Testimonial not found or already processed."]);
        exit;
    }

    $update = $conn->prepare("UPDATE testimonials SET status = 'Denied' WHERE testimonial_id = :id");
    ...
    echo json_encode(["success" => true, "message" => "Testimonial denied successfully."]);
